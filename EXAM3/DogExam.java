...
public class DogExam {
    ...
    // such that all elements in list[0] are Animals, list[1] is Dogs and list[2] is RescueDogs
        ...
        for (int i = 0; i<4; i++){
            for (int j = 0; j< 15; j++){
                ...
                double ranx = (int)(Math.random()*10000)+1; // range is 1-10000
                double rany = (int)(Math.random()*10000)+1;//range is 1-10000
        ...
        System.out.println("Would you like to sort the heights? (0)");
        int choice = input.nextInt();
        if(choice == 0){
            sortHeightA(list);
        }

        System.out.println("Would you like to sort the weights?(0)");
    ...
    public static void display(Animal[][]list){
        /*This method should accept an array as input and display the filled contents of the cart. */
            for (int i=0; i<3; i++){
                for (int j = 0; j< 15; j++){
    ...
    public static void sortHeightA(Animal[][]list){
        int length1 = 3;
        int length2 = 15;
        double arrayD[][] = new double[length1][length2]; //new array filled with numbers that will then be sorted 

        //getting a double from double food
        for (int i =0; i< length1; i++){
            for(int j = 0; j< length2; j++){
                double c = list[i][j].getHeight(); 
                arrayD[i][j] = c; 
            }
        }
        insertionSortA(arrayD);
       
        for (int i =0; i< length1; i++){
            for(int k =0; k<length2; k++){
                for (int j = 0; j<length2; j++){
                    if (arrayD[i][k] == (list[i][j].getHeight())){ //matching up the sorted arrayInt to the original String array bankaccount
                        System.out.println(list[i][j] + " ");
    ...
    public static void sortWeightA(Animal[][]list){
        int length1 = 3;
        int length2 = 15;
        double arrayD[][] = new double[length1][length2]; //new array filled with numbers that will then be sorted 
        ...
        for (int i =0; i< length1; i++){
            for(int k =0; k<length2; k++){
                for (int j = 0; j<length2; j++){
                    if (arrayD[i][k] == (list[i][j].getWeight())){ //matching up the sorted arrayInt to the original String array bankaccount
                        System.out.println(list[i][j] + " ");
                    }//important to note that this NOT permanently edit the Food [] list***
                }
            }
    ...
    public static void insertionSortA(double[][]array1){
        for (int k = 0; k<array1.length; k++){ 
            ...
            for (int i = 1; i<array1[k].length; i++){ //starts at the 1st index (2nd element) because bc [j-1] will cover the 0th index
                int j = i;
                while (j > 0 && (array1[k][j]<  array1[k][j-1])){ //if the variable ahead of the one being compared is LESS then
                    double temp2 = array1[k][j]; //this is the one that is out of order (lesser one in front)
